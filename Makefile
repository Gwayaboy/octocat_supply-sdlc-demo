...
# Supports both template (api-php, api-nodejs, api-python, api-java) and promoted (api) folder structures within actually rendered repositories for easier startup.
...
BACKEND ?= $(shell powershell -Command " \
	if (Test-Path 'api') { \
		if (Test-Path 'api/composer.json') { Write-Output 'php' } \
		elseif (Test-Path 'api/package.json') { Write-Output 'nodejs' } \
	...
	} elseif ((Test-Path 'api-php') -and (Test-Path 'api-nodejs') -and (Test-Path 'api-python') -and (Test-Path 'api-java')) { Write-Output 'nodejs' } \
	elseif (Test-Path 'api-php') { Write-Output 'php' } \
...
BACKEND ?= $(shell \
	if [ -d "api" ]; then \
		if [ -f "api/composer.json" ]; then echo "php"; \
		elif [ -f "api/package.json" ]; then echo "nodejs"; \
		elif [ -f "api/pyproject.toml" ]; then echo "python"; \
		elif [ -f "api/pom.xml" ]; then echo "java"; \
		else echo "unknown"; fi \
	elif [ -d "api-php" ] && [ -d "api-nodejs" ] && [ -d "api-python" ] && [ -d "api-java" ]; then echo "nodejs"; \
	elif [ -d "api-php" ]; then echo "php"; \
...
ifeq ($(BACKEND),php)
	@cd $(API_DIR) && mkdir -p data && ([ -f .env ] || cp .env.example .env) && php artisan key:generate --force && php artisan config:cache && php artisan migrate --force --seed
ifeq ($(OS),Windows_NT)
	npx concurrently --kill-others "cd $(API_DIR) && php artisan serve --port=3000" "cd $(FRONTEND_DIR) && set VITE_API_URL=http://localhost:3000 && npm run dev"
else
	@trap 'kill 0' INT; \
	(cd $(API_DIR) && php artisan serve --port=3000) & \
...
ifeq ($(BACKEND),php)
	cd $(API_DIR) && mkdir -p data && php artisan migrate --force
else ifeq ($(BACKEND),nodejs)
...
ifeq ($(BACKEND),php)
	cd $(API_DIR) && mkdir -p data && php artisan migrate --force --seed
else ifeq ($(BACKEND),nodejs)
...
ifeq ($(BACKEND),php)
	@echo "PHP/Laravel requires no build step"
...
ifeq ($(BACKEND),php)
	cd $(API_DIR) && ./vendor/bin/phpstan analyse --memory-limit=256M 2>/dev/null || echo "PHPStan not configured, skipping"
...
ifeq ($(BACKEND),php)
	cd $(API_DIR) && ./vendor/bin/pint 2>/dev/null || echo "Laravel Pint not installed, skipping"
...
.PHONY: format
format: ## Format code with prettier/pint/ruff
...
ifeq ($(BACKEND),php)
	cd $(API_DIR) && php artisan l5-swagger:generate
else ifeq ($(BACKEND),nodejs)
	cd $(API_DIR) && npm run swagger:generate
else
	@echo "Swagger generation only supported for php and nodejs backends"
endif
...
ifeq ($(BACKEND),php)
	cd $(API_DIR) && php artisan migrate --force --seed && php artisan serve --host=0.0.0.0 --port=3000
else ifeq ($(BACKEND),nodejs)
...
ifeq ($(BACKEND),php)
	$(RM_RF) $(API_DIR)/vendor $(API_DIR)/data
	...
	$(RM_RF) */node_modules */dist */__pycache__ */.venv */target */vendor
endif
ifeq ($(OS),Windows_NT)
	$(RM_RF) $(API_DIR)/*.db
	$(RM_RF) $(API_DIR)/*.db-*
