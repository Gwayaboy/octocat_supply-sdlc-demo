...
<% if demo_options.backend == 'php' %>

Alternatively, if you're in the `api/` directory, you can use Artisan commands directly:
...
3. Run migrations: `make db-migrate`<% if demo_options.backend == 'nodejs' %> (or `cd api && npm run db:migrate`)<% elsif demo_options.backend == 'php' %> (or `cd api && php artisan migrate`)<% endif %>
...
```bash
cd api && ./mvnw spring-boot:run -Dspring-boot.run.arguments="--logging.level.org.hibernate.SQL=DEBUG"
```

<% elsif demo_options.backend == 'php' %>

```bash
cd api && php artisan serve --host=0.0.0.0 --port=3000
```

Enable query logging in `config/database.php` or use Laravel Telescope.
