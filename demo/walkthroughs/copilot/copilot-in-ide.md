...
- **Features:** Copilot Chat (Agent mode), Copilot in VSCode
...
- **Features:** Copilot Vision, Copilot Chat (Agent mode)
...
- **Features:** Copilot custom prompt files
...
- **Features:** Copilot custom instructions
...
### Demo: Discovering and Installing Copilot Plugins from a Marketplace

- **Features:** Copilot plugins (MCP)

> [!NOTE]
> This demo takes approximately 5–10 minutes.

- **What to show:** Browsing the Copilot Plugin Marketplace, installing the Accessibility plugin, optionally running the accessibility agent, and then cleaning up by uninstalling the plugin.
- **Why:** Demonstrate that Copilot's capabilities can be extended with community and first-party plugins, giving developers a single place to discover tools that integrate directly into their AI workflow. The Accessibility plugin is a great example because it provides immediate, visible value by auditing UI components.
- **How:**

  #### In Copilot CLI

  1. Open Copilot CLI in the terminal.
  2. Navigate to the [Octodemo Copilot Plugin Marketplace](https://github.com/msft-common-demos/demo-copilot-plugin-marketplace)
  ...
  4. Browse the available plugins in the marketplace:

     ```bash
     /plugin marketplace browse demo-copilot-plugin-marketplace
     ```

     Walk through the list and highlight the different categories (e.g., code quality, documentation, security, accessibility).

  5. Install the **Accessibility** plugin from the marketplace:

     ```bash
     /plugin install accessibility@demo-copilot-plugin-marketplace
     ```

  6. Verify the plugin is installed and active:

     ```bash
     /plugin list
     ```

  7. **(Optional)** Run the accessibility agent to audit the frontend UI:
  
    ```bash
    run an accessibility audit on the frontend
    ```

    Show how the agent scans the codebase, identifies accessibility issues (missing ARIA labels, contrast problems, keyboard navigation gaps), and suggests fixes. Optionally ask Copilot to apply one or more of the suggested fixes to demonstrate the end-to-end workflow.

  8. Uninstall the plugin to clean up:

     ```bash
     /plugin uninstall accessibility@demo-copilot-plugin-marketplace
     ```

  9.  Confirm the plugin has been removed:
  ...
  1. Open VS Code and navigate to the [**Agent Plugins**](https://code.visualstudio.com/docs/copilot/customization/agent-plugins).
  2. [Configure the plugin marketplace URL](https://code.visualstudio.com/docs/copilot/customization/agent-plugins#_configure-plugin-marketplaces) to point to the demo marketplace: https://github.com/msft-common-demos/demo-copilot-plugin-marketplace
  3. Browse the available plugins and highlight a few categories (e.g., code quality, documentation, security, accessibility).
  4. Search for and select the **Accessibility** plugin. Click **Install** and show the installation confirmation and any permissions the plugin requests.
  5. **(Optional)** Open Copilot Chat and invoke the accessibility agent — for example, ask it to `audit the frontend for WCAG compliance`. Show the results and optionally apply a suggested fix.
  6. Show how to uninstall the plugin from the marketplace panel to clean up.

- **Key Takeaway**: The Copilot Plugin Marketplace provides a centralized way to discover, install, and manage extensions that enhance Copilot across both VS Code and the CLI, enabling teams to tailor their AI assistant to their specific toolchain.

## Security

### Demo: Copilot and Application Security

- **Features:** Copilot Autofix, Code Scanning, GitHub Advanced Security
...
- **Features:** GitHub Actions, Copilot Chat (Agent mode)
...
- **Features:** Copilot custom agents
...
**Video**: [Watch Demo](https://microsofteur.sharepoint.com/:v:/s/octodemo/IQBOc9ehUXQjRrpfIz8AqbfLAdVkrsOxfO2nDx93OPcjWqY?e=kXa4JV&nav=%7B%22playbackOptions%22%3A%7B%22startTimeInSeconds%22%3A1431%7D%7D)
...
- **Features:** Copilot Plan mode


**Video**: [Watch Demo](https://microsofteur.sharepoint.com/:v:/s/octodemo/IQDX59KyXVNjQItdlYgMilXeAT_-_RcrveJPf5X5ZF_NE1E?e=5xEh3d&nav=%7B%22playbackOptions%22%3A%7B%22startTimeInSeconds%22%3A347%7D%7D)
